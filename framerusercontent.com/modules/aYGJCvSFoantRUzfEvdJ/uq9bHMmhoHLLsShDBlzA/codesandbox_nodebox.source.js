            ...
            addListener(eventName, listener) {
                ...
                if (
                    ...
                    this.listenerCount(eventName) >
                        __privateGet(this, _maxListeners) &&
                    ...
                    const memoryLeakWarning =
                        new MemoryLeakError_1.MemoryLeakError(
                            this,
                            eventName,
                            this.listenerCount(eventName)
                        )
...
var ShellProcess = class {
    ...
    async kill() {
        ...
        this.state = "idle"
        await this.channel
            .send("shell/exit", { id: this.id })
            .catch((error) => {
                throw new Error(
                    format('Failed to kill shell with ID "%s"', this.id),
                    { cause: error }
                )
            })
        this.id = void 0
