...
var Stack = class _Stack {
    ...
    reduce(action) {
        ...
        if (depth == 0) {
            this.pushState(
                parser.getGoto(this.state, type, true),
                this.reducePos
            )
        ...
        let base =
            this.stack.length - (depth - 1) * 3 - (action & 262144 ? 6 : 0)
...
var Parse = class {
        ...
        if (this.fragments) {
            ...
            for (let cached = this.fragments.nodeAt(start); cached; ) {
                ...
                if (
                    ...
                    cached.length &&
                    (!strictCx ||
                        (cached.prop(NodeProp.contextHash) || 0) == cxHash)
                ) {
    ...
    runRecovery(stacks, tokens, newStacks) {
        ...
        for (let i = 0; i < stacks.length; i++) {
            ...
            for (let insert of stack.recoverByInsert(token)) {
                if (verbose)
                    console.log(
                        base + this.stackID(insert) + " (via recover-insert)"
                    )
    ...
    stackID(stack) {
        ...
        if (!id2)
            stackIDs.set(
                stack,
                (id2 = String.fromCodePoint(this.nextStackID++))
            )
        return id2 + stack
    }
}
...
var LRParser = class _LRParser extends Parser {
        ...
        this.nodeSet = new NodeSet(
            nodeNames.map((name, i) =>
                NodeType.define({
                    ...
                    error: i == 0,
                    skipped:
                        spec.skippedNodes && spec.skippedNodes.indexOf(i) > -1,
                })
            )
        )
        if (spec.propSources)
    ...
    getName(term) {
        return this.termNames
            ? this.termNames[term]
            : String(
                  (term <= this.maxNode && this.nodeSet.types[term].name) ||
                      term
              )
    }
    ...
    parseDialect(dialect) {
        ...
        for (let i = 0; i < values.length; i++)
            if (!flags[i]) {
                for (
                    let j = this.dialects[values[i]], id2;
                    (id2 = this.data[j++]) != 65535;

                )
                    (disabled || (disabled = new Uint8Array(this.maxTerm + 1)))[
                        id2
                    ] = 1
            }
