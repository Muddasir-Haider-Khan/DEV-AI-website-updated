    ...
    decompose(from, to, target, open) {
        ...
        if (open & 1) {
            ...
            if (joined.length <= 32) {
                ...
                target.push(
                    new _TextLeaf(joined.slice(0, mid)),
                    new _TextLeaf(joined.slice(mid))
                )
            }
        } else {
            target.push(text)
        ...
        function flush() {
            ...
            chunked.push(
                currentChunk.length == 1
                    ? currentChunk[0]
                    : _TextNode.from(currentChunk, currentLen)
            )
            currentLen = -1
...
var SelectionRange = class _SelectionRange {
  ...
  */
    map(change, assoc = -1) {
        ...
        return from == this.from && to == this.to
            ? this
            : new _SelectionRange(from, to, this.flags)
    }
    /**
  ...
  */
    extend(from, to = from) {
        ...
        let head =
            Math.abs(from - this.anchor) > Math.abs(to - this.anchor)
                ? from
                : to
  ...
  */
    asSingle() {
        return this.ranges.length == 1
            ? this
            : new _EditorSelection([this.main], 0)
    }
    /**
...
var EditorState = class _EditorState {
  ...
  */
    changes(spec = []) {
        ...
        return ChangeSet.of(
            spec,
            this.doc.length,
            this.facet(_EditorState.lineSeparator)
        )
    }
    /**
  ...
  */
    toText(string) {
        return Text.of(
            string.split(this.facet(_EditorState.lineSeparator) || DefaultSplit)
        )
    }
    /**
...
var Chunk = class _Chunk {
    ...
    findIndex(pos, side, end, startAt = 0) {
        ...
        for (let lo = startAt, hi = arr.length; ; ) {
            ...
            let diff =
                arr[mid] - pos ||
                (end ? this.value[mid].endSide : this.value[mid].startSide) -
                    side
    ...
    between(offset, from, to, f) {
        for (
            let i = this.findIndex(from, -1e9, true),
                e = this.findIndex(to, 1e9, false, i);
            i < e;
            i++
        )
            if (
  ...
  */
    add(from, to, value) {
        if (!this.addInner(from, to, value))
            (this.nextLayer || (this.nextLayer = new _RangeSetBuilder())).add(
                from,
                to,
                value
            )
...
var SpanCursor = class {
    ...
    next() {
        ...
        if (trackOpen) {
            this.openStart = 0
            for (
                let i = trackOpen.length - 1;
                i >= 0 && trackOpen[i] < from;
                i--
            )
                this.openStart++
        }
