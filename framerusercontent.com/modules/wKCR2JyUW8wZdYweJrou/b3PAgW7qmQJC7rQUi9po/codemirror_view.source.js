                ...
                if (/&/.test(prop)) {
                    render(
                        prop
                            .split(/,\s*/)
                            .map((part) =>
                                selectors.map((sel) => part.replace(/&/, sel))
                            )
...
var DOMSelectionState = class {
    ...
    setRange(range) {
        ...
        this.set(
            ...
            Math.min(
                range.anchorOffset,
                anchorNode ? maxOffset(anchorNode) : 0
            ),
            focusNode,
    ...
    if (toI < children.length) {
        ...
        if (
            ...
            if (
                ...
                if (
                    toOff ||
                    (after.children.length && !after.children[0].length)
                )
    ...
    coordsAt(pos, side) {
        ...
        for (
            ...
            if (
                pos > 0
                    ? i == 0
                    : i == rects.length - 1 || rect.top < rect.bottom
            )
                break
        }
...
function coordsInChildren(view, pos, side) {
    ...
    function scan(view2, pos2) {
        for (
            let i = 0, off = 0;
            i < view2.children.length && off <= pos2;
            i++
        ) {
...
var PointDecoration = class _PointDecoration extends Decoration {
    ...
    range(from, to = from) {
        if (
            this.isReplace &&
            (from > to ||
                (from == to && this.startSide > 0 && this.endSide <= 0))
        )
    ...
    buildText(length, active, openStart) {
        while (length > 0) {
            ...
            this.getLine().append(
                wrapMarks(
                    new TextView(
                        this.text.slice(this.textOff, this.textOff + take)
                    ),
                    active
                ),
                openStart
        ...
        if (deco instanceof PointDecoration) {
            if (deco.block) {
                ...
                if (cursorBefore) {
                    ...
                    openStart =
                        active.length + Math.max(0, openStart - active.length)
                }
            ...
            run: for (;;) {
                if (iI < isolates.length && iScan == isolates[iI].from) {
                    ...
                    if (recurse) {
                        ...
                        let dirSwap =
                            (iso.direction == LTR) != !(localLevel % 2)
                        computeSectionOrder(
                            line,
...
function computeOrder(line, direction, isolates) {
    ...
    if (isolates.length)
        while (line.length > types.length) types[types.length] = 256
    let order = [],
                ...
                if (!rawSel);
                    ...
                    if (browser.gecko) {
                        ...
                        if (nextTo && nextTo != (1 | 2)) {
                            let text = (
                                nextTo == 1 ? textNodeBefore : textNodeAfter
    ...
    if (textAfter && textBefore && textAfter.node != textBefore.node) {
        ...
        if (
            ...
            (descAfter instanceof TextView &&
                descAfter.text != textAfter.node.nodeValue)
        ) {
            textNode = textAfter
            ...
            if (
                !(
                    !descBefore ||
                    (descBefore instanceof TextView &&
                        descBefore.text != textBefore.node.nodeValue)
                )
            )
                textNode = textAfter
        }
            ...
            if (
                ...
                let side = dy
                    ...
                    : dx
                      ...
                      : 0
                closestOverlap =
                    !side || (side > 0 ? i < rects.length - 1 : i > 0)
            }
            if (dx == 0) {
                if (
                    y > rect.bottom &&
        ...
        if (doc2.caretPositionFromPoint) {
            ...
            if (range) {
                ...
                if (
                    ...
                    (browser.chrome &&
                        isSuspiciousChromeCaretResult(node, offset, x))
                )
                    node = void 0
            }
        }
...
var DOMReader = class {
    ...
    readTextNode(node) {
        ...
        for (let point of this.points)
            if (point.node == node)
                point.pos =
                    this.text.length + Math.min(point.offset, text.length)
        for (let off = 0, re = this.lineSeparator ? null : /\r\n?|\n/g; ; ) {
            ...
            this.append(
                text.slice(off, nextBreak < 0 ? text.length : nextBreak)
            )
    ...
    if (
        ...
        if (
            ...
            let compositionRange,
                composition =
                    newSel && findCompositionNode(view, newSel.main.head)
            if (composition) {
            ...
            tr = {
                changes,
                selection:
                    mainSel && startState.selection.replaceRange(mainSel),
            }
        }
    }
    ...
    for (let i = 0; i < sel.ranges.length; i++) {
        ...
        if (from <= pos && to >= pos)
            return EditorSelection.create(
                ...
                sel.mainIndex == i
                    ? 0
                    : sel.mainIndex - (sel.mainIndex > i ? 1 : 0)
            )
    }
    return null
}
...
handlers.beforeinput = (view, event) => {
    ...
    if (
        ...
        if (pending.key == "Backspace" || pending.key == "Delete") {
            let startViewHeight =
                ((_b = window.visualViewport) === null || _b === void 0
                    ? void 0
                    : _b.height) || 0
            setTimeout(() => {
                var _a2
...
var HeightMapText = class _HeightMapText extends HeightMapBlock {
    ...
    replace(_from, _to, nodes) {
        ...
        if (
            nodes.length == 1 &&
            (node instanceof _HeightMapText ||
    ...
    heightMetrics(oracle, offset) {
        ...
        if (oracle.lineWrapping) {
            ...
            if (this.length > lines + 1)
                perChar =
                    (this.height - totalPerLine) / (this.length - lines - 1)
        } else {
    ...
    blockAt(height, oracle, top2, offset) {
        ...
        if (oracle.lineWrapping) {
            let guess =
                ...
                (height < oracle.lineHeight
                    ...
                    : Math.round(
                          Math.max(
                              0,
                              Math.min(1, (height - top2) / this.height)
                          ) * this.length
                      ))
            ...
            let line = Math.max(
                ...
                Math.min(
                    lastLine - firstLine,
                    Math.floor((height - top2) / perLine)
                )
            )
    ...
    forEachLine(from, to, oracle, top2, offset, f) {
        ...
        for (let pos = from, lineTop = top2; pos <= to; ) {
            ...
            if (pos == from) {
                ...
                lineTop +=
                    perLine * linesAbove +
                    perChar * (from - offset - linesAbove)
            }
    ...
    updateHeight(oracle, offset = 0, force = false, measured) {
        ...
        if (
            ...
            if (measured.from > offset)
                nodes.push(
                    new _HeightMapGap(measured.from - offset - 1).updateHeight(
                        oracle,
                        offset
                    )
                )
            ...
            if (pos <= end)
                nodes.push(
                    null,
                    new _HeightMapGap(end - pos).updateHeight(oracle, pos)
                )
...
var HeightMapBranch = class extends HeightMap {
    ...
    lineAt(value, type, oracle, top2, offset) {
        ...
        if (left)
            ...
            return this.left
                .lineAt(rightOffset, subQuery, oracle, top2, offset)
                .join(base2)
    }
    forEachLine(from, to, oracle, top2, offset, f) {
        let rightTop = top2 + this.left.height,
    ...
    for (let parent = dom.parentNode; parent && parent != doc2.body; ) {
        if (parent.nodeType == 1) {
            ...
            if (
                ...
                bottom = Math.min(
                    parent == dom.parentNode ? win.innerHeight : bottom,
                    parentRect.bottom
                )
            }
            parent =
    ...
    draw(viewState, wrapping) {
        return Decoration.replace({
            widget: new LineGapWidget(
                this.displaySize *
                    (wrapping ? viewState.scaleY : viewState.scaleX),
                wrapping
            ),
...
var ViewState = class {
    ...
    viewportIsAppropriate({ from, to }, bias = 0) {
        ...
        let { top: top2 } = this.heightMap.lineAt(
            from,
            QueryType.ByPos,
            this.heightOracle,
            0,
        ...
        let checkLine = (line) => {
            ...
            if (wrapping) {
                ...
                if (target != null) {
                    ...
                    let spaceFrac =
                        ((this.visibleBottom - this.visibleTop) / 2 +
                            marginHeight) /
                        line.height
                ...
                if (totalWidth > 2e6)
                    for (let old of current) {
                        if (
                            ...
                            old.from * this.heightOracle.charWidth +
                                horizOffset <
    ...
    updateLineGaps(gaps) {
        if (!LineGap.same(gaps, this.lineGaps)) {
            ...
            this.lineGapDeco = Decoration.set(
                gaps.map((gap) =>
                    gap.draw(this, this.heightOracle.lineWrapping)
                )
            )
        }
    }
...
var DOMObserver = class {
    ...
    update(update) {
        if (this.editContext) {
            ...
            if (
                update.startState.facet(editable) !=
                update.state.facet(editable)
            )
                update.view.contentDOM.editContext = update.state.facet(
                    editable
                )
...
var EditContextManager = class {
        ...
        this.measureReq = {
            read: (view2) => {
                this.editContext.updateControlBounds(
                    view2.contentDOM.getBoundingClientRect()
                )
        ...
        update.changes.iterChanges((fromA, toA, _fromB, _toB, insert) => {
            ...
            if (toA <= this.from) {
                ...
                if (
                    fromA < this.from ||
                    toA > this.to ||
                    this.to - this.from + insert.length > 3e4
                ) {
                    abort = true
                    return
                }
    ...
    update(update) {
        ...
        if (
            ...
            !this.rangeIsValid(update.state)
        ) {
            this.pendingContextChange = null
...
var EditorView = class _EditorView {
  ...
  */
    static findFromDOM(dom) {
        ...
        return (
            ((_a =
                cView === null || cView === void 0
                    ? void 0
                    : cView.rootView) === null || _a === void 0
                ? void 0
                : _a.view) || null
        )
    }
    ...
    for (let b of bindings) {
        ...
        if (b.any)
            for (let scope of scopes) {
                let scopeObj =
                    bound[scope] ||
                    (bound[scope] = /* @__PURE__ */ Object.create(null))
                if (!scopeObj._any)
                    scopeObj._any = {
                        preventDefault: false,
                        stopPropagation: false,
    ...
    if (
        ...
        if (
            (visualStart || startBlock).to <
                (visualEnd || endBlock).from -
                    (visualStart && visualEnd ? 1 : 0) ||
            (startBlock.widgetLineBreaks > 1 &&
                top2.bottom + view.defaultLineHeight / 2 < bottom.top)
        )
            between.push(piece(leftSide, top2.bottom, rightSide, bottom.top))
        else if (
            top2.bottom < bottom.top &&
            view.elementAtHeight((top2.bottom + bottom.top) / 2).type ==
                BlockType.Text
        )
...
var LayerView = class {
    ...
    update(update) {
        if (
            update.startState.facet(layerOrder) !=
            update.state.facet(layerOrder)
        )
    ...
    markers(view) {
        ...
        for (let r of state.selection.ranges) {
            ...
            if (r.empty || conf.drawRangeCursor) {
                ...
                for (let piece of RectangleMarker.forRange(
                    view,
                    className,
                    cursor
                ))
...
var hideNativeSelection = /* @__PURE__ */ Prec.highest(
...
var dropCursorPos = /* @__PURE__ */ StateField.define({
    ...
    update(pos, tr) {
        ...
        return tr.effects.reduce(
            (pos2, e) => (e.is(setDropCursorPos) ? e.value : pos2),
            pos
        )
    },
})
        ...
        for (let r of view.visibleRanges) {
            ...
            if (to > from) {
                let fromLine = view.state.doc.lineAt(from),
                    toLine =
                        fromLine.to < to ? view.state.doc.lineAt(to) : fromLine
                ...
                if (this.boundary) {
                    for (; from > fromLine.from; from--)
                        if (
                            this.boundary.test(
                                fromLine.text[from - 1 - fromLine.from]
                            )
                        ) {
                            start = from
                            break
                ...
                let ranges = [],
                    m
                let add = (from2, to2, deco2) =>
                    ranges.push(deco2.range(from2, to2))
                if (fromLine == toLine) {
                    ...
                    iterMatches(
                        ...
                        (from2, m2) => this.addMatch(m2, view, from2, add)
                    )
                }
                deco = deco.update({
                    filterFrom: start,
...
var plugin = /* @__PURE__ */ ViewPlugin.fromClass(
    class {
        constructor() {
            this.height = 1e3
...
function crosshairCursor(options = {}) {
    ...
    let plugin2 = ViewPlugin.fromClass(
        class {
            constructor(view) {
                this.view = view
                this.isDown = false
    ...
    return [
        ...
        EditorView.contentAttributes.of((view) => {
            var _a
            return (
                (_a = view.plugin(plugin2)) === null || _a === void 0
                    ? void 0
                    : _a.isDown
            )
                ? showCrosshair
...
var TooltipViewManager = class {
    constructor(view, facet, createTooltipView, removeTooltipView) {
        ...
        let prev = null
        this.tooltipViews = this.tooltips.map(
            (t) => (prev = createTooltipView(t, prev))
        )
    }
    update(update, above) {
        var _a
        ...
        for (let i = 0; i < tooltips2.length; i++) {
            ...
            if (known < 0) {
                tooltipViews[i] = this.createTooltipView(
                    tip,
            ...
            position: browser.ios
                ...
                : ((_a = values.find((conf) => conf.position)) === null ||
                  _a === void 0
                      ...
                      : _a.position) || "fixed",
            parent:
                ((_b = values.find((conf) => conf.parent)) === null ||
                _b === void 0
                    ? void 0
                    : _b.parent) || null,
            tooltipSpace:
...
var tooltipPlugin = /* @__PURE__ */ ViewPlugin.fromClass(
    class {
        ...
        destroy() {
            ...
            if (this.parent) this.container.remove()
            ;(_b = this.resizeObserver) === null || _b === void 0
                ? void 0
                : _b.disconnect()
        ...
        readMeasure() {
            ...
            if (this.position == "fixed" && this.manager.tooltipViews.length) {
                ...
                if (browser.gecko) {
                    ...
                    let rect = dom.getBoundingClientRect()
                    makeAbsolute =
                        Math.abs(rect.top + 1e4) > 1 || Math.abs(rect.left) > 1
                }
            }
            ...
            for (let i = 0; i < this.manager.tooltips.length; i++) {
                ...
                if (
                    ...
                    (tooltip.clip !== false &&
                        (pos.bottom <= Math.max(visible.top, space.top) ||
                            ...
                            pos.right <
                                Math.max(visible.left, space.left) - 0.1 ||
                            pos.left >
                                Math.min(visible.right, space.right) + 0.1))
                ) {
                    dom.style.top = Outside
                    continue
                }
                ...
                if (
                    ...
                    (above
                        ...
                        : pos.bottom + height + arrowHeight + offset.y >
                          space.bottom) &&
...
var HoverTooltipHost = class _HoverTooltipHost {
    ...
    createHostedView(tooltip, prev) {
        ...
        this.dom.insertBefore(
            hostedView.dom,
            prev ? prev.dom.nextSibling : this.dom.firstChild
        )
...
var showHoverTooltipHost = /* @__PURE__ */ showTooltip.compute(
    [showHoverTooltip],
    (state) => {
...
var HoverPlugin = class {
    ...
    get tooltip() {
        ...
        let index = plugin2
            ? plugin2.manager.tooltips.findIndex(
    ...
    mousemove(event) {
        ...
        if (
            ...
            let { pos } = active[0] || this.pending,
                    ...
                    (_b =
                        (_a = active[0]) === null || _a === void 0
                            ? void 0
                            : _a.end) !== null && _b !== void 0
                        ? _b
                        : pos
            if (
                pos == end
...
var PanelGroup = class {
    ...
    sync(panels2) {
        for (let p of this.panels)
            if (p.destroy && panels2.indexOf(p) < 0) p.destroy()
        this.panels = panels2
        this.syncDOM()
    }
    syncDOM() {
            ...
            { formatNumber: String, domEventHandlers: {} },
            {
                domEventHandlers(a, b) {
                    ...
                    for (let event in b) {
                        ...
                        result[event] = exists
                            ? (view, line, event2) =>
                                  exists(view, line, event2) ||
                                  add(view, line, event2)
                            : add
                    }
                    return result
                },
...
var activeLineGutterHighlighter = /* @__PURE__ */ gutterLineClass.compute(
    ["selection"],
    (state) => {
        let marks = [],
            last = -1
...
var whitespaceHighlighter = /* @__PURE__ */ matcher(
...
var trailingHighlighter = /* @__PURE__ */ matcher(
    /* @__PURE__ */ new MatchDecorator({
        regexp: /\s+$/g,
